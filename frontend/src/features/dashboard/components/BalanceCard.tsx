...
/**
 * Balance Card — pulls saldo & monthly net from GET /api/dashboard.
 */
export function BalanceCard() {
  const { data, isPending, error } = useDashboardQuery()

  const balanceInCents = data?.total_balance_cents ?? 0
  const monthlyChangeInCents = data?.monthly_net_cents ?? 0
  const isPositiveChange = monthlyChangeInCents > 0

  if (error) {
    return (
      <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-sm text-red-700">
        Gagal memuat saldo. Pastikan API berjalan dan Anda sudah login.
      </div>
    )
  }
  ...
  if (isPending) {
    return (
      <div className="animate-pulse rounded-xl border border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-100 p-8">
        <div className="h-6 w-32 rounded bg-white/60" />
        <div className="mt-4 h-10 w-48 rounded bg-white/80" />
        <div className="mt-2 h-4 w-56 rounded bg-white/50" />
      </div>
    )
  }
            ...
            <div className="flex items-center gap-2">
              <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-sm font-medium ${
                  isPositiveChange
                    ...
                    : monthlyChangeInCents < 0
                      ? 'bg-red-100 text-red-700'
                      : 'bg-gray-100 text-gray-700'
                }`}>
                <svg className={`w-4 h-4 ${isPositiveChange ? 'rotate-0' : monthlyChangeInCents < 0 ? 'rotate-180' : ''}`}
                  fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 17l9.2-9.2M17 17V7M17 17H7" />
                </svg>
                {monthlyChangeInCents !== 0 && (monthlyChangeInCents > 0 ? '+' : '')}
                {formatRupiah(Math.abs(monthlyChangeInCents))}
              </div>
